...
# from .unet import UNet, UNetTriplane
from .unet_3daware import setup_unet #UNetTriplane3dAware
# from .geometry_predictor import MappingNetwork, SynthesisNetworkWithInput
from .conv_pointnet import ConvPointnet

# from src.utils.ops import grid_sample_3d
...
def sample_triplane_feat(feature_triplane, normalized_pos):
    ...
    # normalized_pos = torch.clamp(position, 0, 1)
...
class TriPlanePC2Encoder(torch.nn.Module):
    ...
    def __init__(
        ...
        # self.max_logsigma = cfg.max_logsigma
        # self.min_logsigma = cfg.min_logsigma
    ...
    def encode(self, point_cloud_xyz, point_cloud_feature, mv_feat=None, pc2pc_idx=None) -> Dict:
        ...
        if self.cfg.point_encoder_type == 'pvcnn':
            ...
            # TODO cleanup share scatter code
            if self.cfg.use_point_scatter:
                ...
                # TODO: should incorporate voxel features as well??
                # Reduce voxel features to triplane
                # xy, yz, xz
                pc_feat = pc_feat[0]
        ...
        # feat_dim = pc_feat_1.shape[1]  # feat dim 
        # z_dim = feat_dim // 2
        # z_1_mean, z_1_logsigma = pc_feat_1[:, :z_dim], pc_feat_1[:, z_dim:]
        # z_2_mean, z_2_logsigma = pc_feat_2[:, :z_dim], pc_feat_2[:, z_dim:]
        # z_3_mean, z_3_logsigma = pc_feat_3[:, :z_dim], pc_feat_3[:, z_dim:]

        # z_logsigma = torch.stack([z_1_logsigma, z_2_logsigma, z_3_logsigma], dim=1)
        # z_logsigma = softclip(z_logsigma, min=self.min_logsigma, max=self.max_logsigma)
        
        # # output  is [B,3,C,H,W]
        # output.update({
        #     'z_mean': torch.stack([z_1_mean, z_2_mean, z_3_mean], dim=1),
